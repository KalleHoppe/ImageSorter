    ...
    public class ColourScaler : Filter
    {
        ...
        private void Process(RawImage ri)
        {
            ...
            if (use_camera_wb && cam_mul[0] != -1)
            {
                ...
                {
                    Console.WriteLine("{0}: Cannot use camera white balance.", filename);
                }
            }
            ...
            if (verbose)
            {
                Console.Write("Scaling with darkness {0}, saturation {1}, and\nmultipliers", dark, sat);

                for (c = 0; c < 4; c++)
                {
                    Console.Write(" {0}", pre_mul[c]);
                }
                Console.WriteLine();
            }
            ...
            if ((aber[0] != 1 || aber[2] != 1) && colors == 3)
            {
                if (verbose)
                {
                    Console.WriteLine("Correcting chromatic aberration...");
                }

...
#if DEBUG_HUFFMANTREE
            public void DumpTable(int indent)
            {
                Console.WriteLine(new string(' ', indent * 2) + leaf);
                if (branch[0] != null) branch[0].DumpTable(indent + 1);
                if (branch[1] != null) branch[1].DumpTable(indent + 1);
            }
#endif
        }
    }
